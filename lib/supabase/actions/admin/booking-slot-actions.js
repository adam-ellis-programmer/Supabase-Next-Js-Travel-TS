...
export async function updateBookingDates(
  ...
  try {
    ...
    // gather up ids and compare ( practice )
    // gather up ids and compare ( practice )
    // get the ids and check for !has and has
    // existingSlots from db and  bookingSlotsData

    async function detectAndDeleteDates() {
      const deleteDatesIds = new Set()
      console.log('\n running from detectDeleted dates')

      for (const slot of existingSlots) {
        console.log(slot.booking_slot_dates)
        deleteDatesIds.add(slot.id)
      }
      console.log(deleteDatesIds)
    }
